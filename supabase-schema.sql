...
-- ── 1. TRIGGER: criar perfil ao registrar usuário ────────────────────────────
...
create policy "Admin gerencia condomínios"
  on public.condominios for all using (
    exists (select 1 from public.profiles where id = auth.uid() and tipo = 'administrador')
  );
...
create policy "Admin e Síndico gerenciam moradores"
  on public.moradores for all using (
    exists (select 1 from public.profiles where id = auth.uid() and tipo in ('administrador','sindico'))
  );
...
create policy "Gestores atualizam chamados"
  on public.tickets for update using (
    exists (select 1 from public.profiles where id = auth.uid() and tipo in ('administrador','sindico','colaborador'))
  );
...
-- ── 5. APÓS CRIAR OS USUÁRIOS NO AUTH ────────────────────────────────────────
-- Execute os comandos abaixo DEPOIS de criar os usuários em:
-- Authentication → Users → Add User
--
-- update public.profiles set tipo='administrador', cpf='456', nome='Dra. Cristhiane Xavier', unidade='Sede Administrativa', condominio_id='cd-1' where email='admin@facilities.com.br';
-- update public.profiles set tipo='sindico',       cpf='789', nome='Carlos Eduardo',         unidade='Apto 01-A',           condominio_id='cd-1' where email='sindico@facilities.com.br';
-- update public.profiles set tipo='morador',       cpf='123', nome='Roberto Silva',           unidade='Apto 41-B',           condominio_id='cd-1' where email='contato@facilities.com.br';
-- update public.profiles set tipo='colaborador',   cpf='101', nome='Lucas Ferreira',          unidade='Portaria',            condominio_id='cd-1' where email='colaborador@facilities.com.br';
-- update public.profiles set tipo='porteiro',      cpf='102', nome='Marcos Porteiro',         unidade='Guarita',             condominio_id='cd-1' where email='porteiro@facilities.com.br';
